...
const StatusRemixerScreen = () => {
  ...
  const handleShare = async () => {
    if (!selectedImage?.uri || !memeResult) return;
    try {
      await Share.open({
        url: selectedImage.uri,
        message: `${memeResult.captionTop}\n${memeResult.captionBottom}`,
      });
    } catch (_) {}
  };
    ...
    <ScrollView contentContainerStyle={styles.container}>
      ...
      {selectedImage && (
        <View style={styles.memeContainer}>
          ...
          />
          {memeResult && (
            <Text style={[styles.memeText, styles.memeTextTop]}>
              {memeResult.captionTop}
            </Text>
          )}
          {memeResult && (
            <Text style={[styles.memeText, styles.memeTextBottom]}>
              {memeResult.captionBottom}
            </Text>
          )}
          {loading && (
            <View style={styles.loadingOverlay}>
              ...
              <Text style={styles.loadingText}>L'IA analyse ton image... 🤖</Text>
            </View>
          )}
        </View>
      )}
